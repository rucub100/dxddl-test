...
public final class DirectVertex implements AutoCloseable {
    ...
    public static final void createReserved(final long[] p_reserved_cids) {
        Stopwatch.GLOBAL.split();
        final int size = size();
        final int length = p_reserved_cids.length;
        final int[] sizes = new int[length];
        for (int i = 0; i < length; i++) {
            sizes[i] = size;
        }
        Stopwatch.GLOBAL.split("SIZES");
        int created = CHUNK_LOCAL_SERVICE.createReservedLocal().create(p_reserved_cids, length, sizes);
        if (created != length) {
            throw new RuntimeException(String.format("Failed to create %d chunks!", length));
        }
        Stopwatch.GLOBAL.split("CREATE RESERVED LOCAL");
        // initialize with default values
        for (int i = 0; i < length; i++) {
            final long address = CHUNK_LOCAL_SERVICE.pinningLocal().pin(p_reserved_cids[i]).getAddress();
            //CHUNK_LOCAL_SERVICE.rawWriteLocal().writeInt(address, OFFSET_DEPTH, -1);
            CHUNK_LOCAL_SERVICE.rawWriteLocal().writeInt(address, OFFSET_NEIGHBORS_LENGTH, 0);
            //CHUNK_LOCAL_SERVICE.rawWriteLocal().writeLong(address, OFFSET_NEIGHBORS_CID, ChunkID.INVALID_ID);
            //CHUNK_LOCAL_SERVICE.rawWriteLocal().writeLong(address, OFFSET_NEIGHBORS_ADDR, Address.INVALID);
            //CHUNK_LOCAL_SERVICE.pinningLocal().unpinCID(p_reserved_cids[i]);            
        }
        Stopwatch.GLOBAL.split("DEFAULT VALUES");
    }
    ...
    public static void remove(final long[] p_cids) {
        ...
        CHUNK_SERVICE.remove().remove(p_cids);
    }
