 ...
 *
 */
public final class TestCase6 extends AbstractTest {

    private static final Logger LOGGER = LogManager.getFormatterLogger(TestCase6.class);
    ...
    protected void runViaRegularAccess(
        ...
        for (long i = 0; i < numOfOps; i++) {
            ...
            if (testChunk.getNumbers()[1000] < min) {
                min = testChunk.getNumbers()[1000];
            }
        }
    }
    ...
    protected void runViaDirectAccess(final int numOfChunks, final int numOfOps, final long[] ids) {
        ...
        for (int i = 0; i < numOfOps; i++) {
            num = DirectTestChunk6.getNumbers(ids[i % numOfChunks], 0);
            if (num < min) {
                min = num;
            }
        }
    ...
    public String getName() {
        return "Test case 6 (" + new TestChunk6().sizeofObject() + ")";
    }
}
